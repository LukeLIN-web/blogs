...
| **特性**       | **BPE (字节对编码)**                                       | **Unigram 模型**                                         |
| -------------- | ---------------------------------------------------------- | -------------------------------------------------------- |
| **训练机制**   | **贪婪、确定性**。重复合并最高频的相邻字节对。             | **迭代、随机性**。使用 EM 算法，对初始化和局部最优敏感。 |
| **结果一致性** | **高**。给定语料和词表大小，结果几乎总是相同的。           | **低**。对初始化、超参、语料顺序敏感，结果不稳定。       |
| **可控性**     | **高**。通过精确控制合并次数或词表大小来精确控制子词粒度。 | **低**。结果依赖于复杂的概率分布和迭代过程。             |

解决 OOV (Out-of-Vocabulary) 问题,  BPE 将词汇分解为更小的**子词 (Subword)**。

- 例如，一个新词 **"unbelievability"** 即使从未在训练集中出现过，BPE 也可以将其分解为它认识的子词，如 `un`, `believe`, `ability`，或者更细的粒度。

**小词表 (如字符级)：** 模型无法高效地捕获词汇语义，序列长度过长，计算量大。

**大词表 (如词级)：** OOV 问题严重，词表维护成本高。
